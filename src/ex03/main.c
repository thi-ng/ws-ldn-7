...
DemoFn demos[] = { demoWelcome, demoGraph, demoScribble, demoGUI };
uint32_t demoID = 0;
...
static SpriteSheet dialSheet = { .pixels = bt_dustknob48_12_rgb888,
		.spriteWidth = 48, .spriteHeight = 48, .numSprites = 12, .format = CM_RGB888 };
...
static float demoFunction__(float theta) {
	...
	y += 0.2f * sinf(5.0f * theta);
	y += 0.1666f * sinf(6.0f * theta);
	return y * 0.6;
}
...
static void initAppGUI() {
	gui = initGUI(3, &UI_FONT, 0xff59626c, 0xffffffff);
	gui->items[0] = guiDialButton(0, "Volume", 135, 100, 0.0f, 0.025f, &dialSheet, NULL);
	gui->items[1] = guiDialButton(1, "Freq", 205, 100, 0.0f, 0.025f, &dialSheet, NULL);
	gui->items[2] = guiDialButton(2, "Filter", 275, 100, 0.0f, 0.025f,
			&dialSheet,
			NULL);
}
