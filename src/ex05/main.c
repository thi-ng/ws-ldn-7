		...
		while (numPackets--) {
			...
			if ((type & 0xf0) == 0xb0) {
				...
				case MIDI_CC_KNOB5:
					...
					break;
				case MIDI_CC_KNOB6:
					preset->impFreq = expf(4.5f * val / 127.0f - 3.5f) / 2.7f * 40.0f;
					break;

				default:
					if (subtype >= MIDI_CC_BT_S1 && subtype <= MIDI_CC_BT_S8) {
						int8_t idx = subtype - MIDI_CC_BT_S1;
						int8_t note = tracks[0]->notes[idx];
						if (note == -1) {
							tracks[0]->notes[idx] = scale[tinymt32_generate_uint32(&rng) % 16];
						} else {
							tracks[0]->notes[idx] = -1;
						}
					}
					if (subtype >= MIDI_CC_BT_M1 && subtype <= MIDI_CC_BT_M8) {
						int8_t idx = subtype - MIDI_CC_BT_M1;
						int8_t note = tracks[1]->notes[idx];
						if (note == -1) {
							tracks[1]->notes[idx] = scale[tinymt32_generate_uint32(&rng) % 16];
						} else {
							tracks[1]->notes[idx] = -1;
						}
					}
					break;
				}
